<template>
    <NuxtLayout name="inner">
                  ...
                  <nav aria-label="breadcrumb">
                    <ol class="breadcrumb mb-0 p-0 align-items-center">
                      ...
                      <li class="breadcrumb-item active" aria-current="page">
                        <Translate text="Generate billing"/>
                      </li>
                    </ol>
                  </nav>
                </div>
              ...
              <div>
                ...
                <h6 class="mb-0 text-uppercase" v-if="possiblespecimens.length>0">
                  <Translate text="PATIENT:"/>&nbsp; {{ possiblespecimens[0].patient }}
                </h6>
                <hr/>
                <br/>  <br/> 
                <p>
                  <strong>
                    <Translate text="Choose the different test(s) from below:"/>
                  </strong>
                </p>
                ...
                <table class="table table-striped table-bordered">
                    <thead>
                        <tr role="row">
                            <th rowspan="1" colspan="1"><Translate text="Test Name"/></th>
                            <th rowspan="1" colspan="1"><Translate text="Specimen Type"/></th>
                            <th rowspan="1" colspan="1"><Translate text="Test Cost"/></th>
                            <th rowspan="1" colspan="1"><Translate text="Discount"/></th>
                            <th rowspan="1" colspan="1"><Translate text="Discount Amount"/></th>
                            <th rowspan="1" colspan="1"><Translate text="Total"/></th>
                        </tr>
                    </thead>
                    <tbody>
                        <tr v-if="selectedspecimens.length==0">
                            <td colspan="5">
                              <center>
                                <strong>
                                  <Translate text="Please select a test."/>
                                </strong>
                              </center>
                            </td>
                        </tr>
                        ...
                        <tr v-for="(i,j) in selectedspecimens">
                            ...
                            <td>
                                <select v-model="selectedspecimens[j].discounttype">
                                    ...
                                    <option value="PERC"><Translate text="Percentage"/></option>
                                    <option value="FLAT"><Translate text="Flat"/></option>
                                </select>
                            </td>
                            <td>
                            ...
                            <td>
                                <strong v-if="selectedspecimens[j].discounttype=='PERC'">
                                  {{ selectedspecimens[j].amount - ((selectedspecimens[j].amount*(selectedspecimens[j].discountamount??0))/100) }} {{ curr }}
                                </strong>
                                <strong v-if="selectedspecimens[j].discounttype=='FLAT'">
                                  {{ selectedspecimens[j].amount - (selectedspecimens[j].discountamount??0) }} {{ curr }}
                                </strong>
                                <strong v-if="selectedspecimens[j].discounttype=='NONE'||!selectedspecimens[j].discounttype">
                                  {{ selectedspecimens[j].amount }} {{ curr }}
                                </strong>
                            </td>
                        </tr>
                    </tbody>
                    ...
                    <tfoot>
                        <tr role="row">
                            <th rowspan="1" colspan="5"><Translate text="Total Amount"/></th>
                ...
                <button v-if="selectedspecimens.length>0" class="btn btn-primary w-100 mt-5" @click="save">
                  &check; <Translate text="Create bill"/>
                </button>
            </div>
        ...
        methods:{
            ...
            save(){
                if(this.selectedspecimens.length==0){
                    return errorToast(this.$t("You must choose a specimen"));
                }
                const context=this;
                this.bill.meta={
